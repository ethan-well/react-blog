...
class ArticleNav extends React.Component {
  ...
  onTabChange = (key) => {
    ...
    const url2 = `api/categories/${key}/articles`
  ...
  componentWillMount() {
    ...
    const url2 = `api/categories/${this.state.current_key}/articles`
    HttpHandler.GetHandler(url2, this.initArticleList);
  }
  ...
  componentWillReceiveProps(nextprops) {
    if(nextprops.categoryId !== this.props.category_id) {
      ...
      const url2 = `api/categories/${nextprops.categoryId}/articles`
      HttpHandler.GetHandler(url2, this.initArticleList);
    }
  }
